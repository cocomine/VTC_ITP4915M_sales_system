...
namespace UI.Delivery_Page {
    public partial class Delivery_Team_Page : Form {
        ...
        private void Delivery_Page_Load(object sender, EventArgs e) {
            ...
            MySqlCommand cmd_order = new MySqlCommand("SELECT * FROM `delivery` AS d, `customer` AS c, `delivery_team_staff` AS dts " + "WHERE dts.StaffAccountID = '" + acc.Get_acoountID() + "' AND c.CustomerID = d.CustomerID AND d.Status = '1' AND dts.Delivery_TeamID = d.Delivery_TeamID;", conn);
        ...
        private void btn_complete_Click(object sender, EventArgs e) {
            ...
            MySqlCommand cmd_comp = new MySqlCommand("UPDATE `delivery` AS d SET d.Status = '2' " + "WHERE d.OrderID = '" + lb_order.Text + "';", conn);
            MySqlCommand update_dTeamStatus = new MySqlCommand("UPDATE `delivery_team` AS ds, `delivery` AS d SET ds.Status = '0' " + "WHERE d.OrderID = '" + lb_order.Text + "' AND ds.TeamID = d.Delivery_TeamID;", conn);
